...
public interface ConnectorMetadata
{
    ...
    /**
     * Do whatever is necessary to start an MERGE query, returning the {@link ConnectorMergeHandle}
     * instance that will be passed to the PageSink, and to the {@link #finishMerge} method.
     */
    default ConnectorMergeHandle beginMerge(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        throw new PrestoException(NOT_SUPPORTED, MODIFYING_ROWS_MESSAGE);
    }
     ...
     *
     * @deprecated Use {@link #finishMerge(ConnectorSession, ConnectorMergeHandle, List, Collection, Collection)}
     */
    @Deprecated
    default void finishMerge(ConnectorSession session, ConnectorMergeHandle mergeTableHandle, Collection<Slice> fragments, Collection<ComputedStatistics> computedStatistics)
    {
     ...
     *
     * @param session The session
     * @param mergeHandle A ConnectorMergeHandle for the table that is the target of the merge
    ...
    default void finishMerge(
            ConnectorSession session,
            ConnectorMergeHandle mergeHandle,
            List<ConnectorTableHandle> sourceTableHandles,
            Collection<Slice> fragments,
            Collection<ComputedStatistics> computedStatistics)
    {
        // Delegate to deprecated SPI to not break existing connectors
        finishMerge(session, mergeHandle, fragments, computedStatistics);
    }
